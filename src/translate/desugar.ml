
open Lang
open Ast
open Constraints
open Expr
open Pattern
open Translation_tools
open Ast_tools
open Ast_tools.Utils

module LetMonad = struct
  module Binding = struct
    module Ty = struct
      type t =
        | Untyped
        | Typed of { do_check : bool ; tau : Desugared.t }

      let typed_of_tau_opt ?(do_check : bool = true) (tau_opt : Desugared.t option) : t =
        match tau_opt with
        | Some tau -> Typed { do_check ; tau }
        | None -> Untyped
    end

    type t =
      { ty   : Ty.t
      ; var  : Ident.t
      ; defn : Desugared.t
      }

    type a = Constraints.desugared

    let t_to_expr { ty ; var ; defn } ~body =
      match ty with
      | Untyped -> ELet { var ; defn ; body }
      | Typed { tau ; do_check } ->
        ELetTyped
          { typed_var = { var ; tau }
          ; defn
          ; body
          ; typed_binding_opts = TBDesugared { do_check ; do_wrap = true }
          }
  end

  include Let_builder (Binding)

  (*
    Assign the expression the given name with optional typing.
  *)
  let assign ?(ty : Binding.Ty.t = Untyped) (var : Ident.t) (defn : Desugared.t) : unit m =
    tell { ty ; var ; defn }
end

let desugar_pgm (names : (module Fresh_names.S)) (pgm : Bluejay.pgm)
    ~(do_type_splay : Splay.t) : Desugared.pgm =
  let module Names = (val names) in
  let open LetMonad in

  let rec desugar (expr : Bluejay.t) : Desugared.t =
    match expr with
    (* Base cases *)
    | (EInt _ | EBool _ | EVar _ | EInput | ETypeInt | ETypeBool
      | EType | ETypeTop | ETypeBottom | ETypeSingle | ETypeUnit | EUnit) as e -> e
    (* Simple propogation *)
    | EBinop { left ; binop ; right } ->
      begin match binop with
        | BAnd -> EIf { cond = desugar left ; true_body = desugar right ; false_body = EBool false }
        | BOr -> EIf { cond = desugar left ; true_body = EBool true ; false_body = desugar right }
        | (BDivide | BModulus) when match right with EInt _ -> false | _ -> true -> (* protect with branch when right is anything but an int literal *)
          build @@
          let v = Names.fresh_id () in
          let* () = assign v @@ desugar right in
          return @@ EIf
            { cond = EBinop { left = EVar v ; binop = BEqual ; right = EInt 0 }
            ; true_body = EAbort "Divide or modulo by 0"
            ; false_body = EBinop { left = desugar left ; binop ; right = EVar v } }
        | _ -> EBinop { left = desugar left ; binop ; right = desugar right }
      end
    | EIf { cond ; true_body ; false_body } ->
      EIf { cond = desugar cond ; true_body = desugar true_body ; false_body = desugar false_body }
    | ELet { var ; defn ; body } ->
      ELet { var ; defn = desugar defn ; body = desugar body }
    | EAppl { func ; arg } ->
      EAppl { func = desugar func ; arg = desugar arg }
    | EProject { record ; label } ->
      EProject { record = desugar record ; label }
    | ENot e ->
      ENot (desugar e)
    | EDefer e ->
      EDefer (desugar e)
    | EFunction { param ; body } ->
      EFunction { param ; body = desugar body }
    | EVariant { label ; payload } ->
      EVariant { label ; payload = desugar payload }
    | ERecord m ->
      ERecord (RecordLabel.Map.map desugar m)
    | ETypeFun { domain ; codomain ; dep } ->
      ETypeFun { domain = desugar domain ; codomain = desugar codomain ;  dep }
    | ETypeRecord m ->
      ETypeRecord (RecordLabel.Map.map desugar m)
    | ETypeModule m ->
      ETypeModule (List.map (fun (label, e) -> label, desugar e) m)
    | ETypeRefinement { tau ; predicate } ->
      ETypeRefinement { tau = desugar tau ; predicate = desugar predicate }
    | ETypeMu { var ; params ; body } ->
      ETypeMu { var ; params ; body = desugar body }
    | ETypeVariant ls_e ->
      ETypeVariant (List.map (fun (label, e) -> label, desugar e) ls_e)
    | ELetTyped
        { typed_var = { var ; tau }; defn; body;
          typed_binding_opts = TBBluejay } ->
      ELetTyped
        { typed_var = { var ; tau = desugar tau }
        ; defn = desugar defn
        ; body = desugar body
        ; typed_binding_opts = TBDesugared { do_check = true; do_wrap = true }
        }
    (* Assert/assume *)
    | EAssert assert_expr ->
      mk_assert
        (desugar assert_expr)
        (Format.sprintf "Failed assertion: %s" (Expr.to_string assert_expr))
    | EAssume assume_expr ->
      mk_assume (desugar assume_expr)
    (* Dependent records / modules *)
    | EModule stmts -> EModule (List.concat_map desugar_statement stmts)
    (* Patterns *)
    | EMatch { subject ; patterns } ->
      let patterns =
        List.map (fun (pat, e) -> desugar_pattern pat e) patterns
      in
      EMatch { subject = desugar subject ; patterns }
    (* Lists *)
    | EList [] ->
      EVariant { label = Reserved.nil ; payload = EUnit }
    | EList ls_e ->
      desugar
      @@ List.fold_right (fun e acc ->
          EListCons (e, acc)
        ) ls_e (EList [])
    | EListCons (e_hd, e_tl) ->
      let bindings =
        [ (Reserved.hd, desugar e_hd)
        ; (Reserved.tl, EAppl { func = Desugared_functions.filter_list ; arg = desugar e_tl })
        ]
      in
      let payload = ERecord (Parsing_tools.record_of_list bindings) in
      EVariant { label = Reserved.cons ; payload }
    | ETypeList ->
      let tau = Names.fresh_id ~suffix:"tau_list" () in
      let t = Names.fresh_id ~suffix:"list_t" () in
      abstract_over_ids [tau] @@
      let list_constructors =
        [ (Reserved.nil_type, ETypeUnit)
        ; (Reserved.cons_type,
           ETypeRecord
             (Parsing_tools.record_of_list
                [ (Reserved.hd, EVar tau)
                ; (Reserved.tl, EVar t)
                ]
             )
          )
        ]
      in
      ETypeMu { var = t ; params = [] ; body = ETypeVariant list_constructors }
    (* Intersection type *)
    | ETypeIntersect ls_e ->
      desugar @@
      let x = Names.fresh_id ~suffix:"x_match_type" () in
      let catch_label label =
        let variant_label = VariantTypeLabel.to_variant_label label
        and payload_id = Reserved.catchall in
        PVariant { variant_label ; payload_id }
      in
      let patterns =
        List.map (fun (label, _, tau') -> catch_label label, tau') ls_e
      in
      ETypeFun
        { domain = ETypeVariant (List.map (fun (label, tau, _) -> label, tau) ls_e)
        ; codomain = EMatch { subject = EVar x ; patterns }
        ; dep = `Binding x
        }
    (* Abstract type *)
    | EAbstractType -> EGen EType
    (* Functions *)
    | EMultiArgFunction { params ; body } ->
      abstract_over_ids params (desugar body)
    | ELetFun { func ; body } -> begin
        let { Function_components.func_id ; defn ; params ; tau_opt } =
          funsig_to_components func
        in
        build @@
        let* () =
          assign ~ty:(Binding.Ty.typed_of_tau_opt tau_opt) func_id (
            abstract_over_ids params defn
          )
        in
        return (desugar body)
      end
    | ELetFunRec { funcs ; body } ->
      (* just roll up the statements and tell it to finish with body *)
      pgm_to_expr_with_body (desugar body)
      @@ desugar_rec_funs_to_stmt_list funcs

  (*
    Breaks a function signature into its id, type (optional), parameter names, and function body, all desugared.
  *)
  and funsig_to_components (fsig : Bluejay.funsig) : desugared Function_components.t =
    Function_components.map ~f:desugar
    @@ Funsig.to_components fsig

  (*
    Desugars list patterns into the associated variants and also returns the
    new body because some projections are necessary.
  *)
  and desugar_pattern (pat : Bluejay.pattern) (e : Bluejay.t) : Desugared.pattern * Desugared.t =
    match pat with
    | PEmptyList ->
      PVariant
        { variant_label = Reserved.nil
        ; payload_id = Reserved.catchall }
    , desugar e
    | PDestructList { hd_id ; tl_id } ->
      let r = Names.fresh_id () in
      PVariant
        { variant_label = Reserved.cons
        ; payload_id = r }
    , build @@
      let* () = assign hd_id (EProject { record = EVar r ; label = Reserved.hd }) in
      let* () = assign tl_id (EProject { record = EVar r ; label = Reserved.tl }) in
      return (desugar e)
    | (PAny | PVariable _ | PVariant _) as p -> p, desugar e

  (*
    Turns mutually recursive functions into many statements.
    This is useful for both desugaring statements and expressions.
  *)
  and desugar_rec_funs_to_stmt_list (fsigs : bluejay funsig list) : Desugared.statement list =
    match fsigs with
    | [ (FTyped _) as f ] when Splay.is_yes do_type_splay ->
      let comps = funsig_to_components f in
      let tau = Option.get comps.tau_opt in
      let typed_binding_opts = TBDesugared { do_wrap = true ; do_check = true }
      and defn =
        apply
          (abstract_over_ids (comps.func_id :: comps.params) comps.defn)
          (EGen tau)
      and typed_var = { var = comps.func_id ; tau } in
      let stmt = Expr.STyped { typed_var ; typed_binding_opts ; defn } in
      [ stmt ]
    | _ ->
      let func_comps = List.map funsig_to_components fsigs in
      let f_names = List.map (fun r -> r.Function_components.func_id) func_comps in
      let r = Names.fresh_id ~suffix:"r" () in
      let defns =
        List.map (fun comps ->
            abstract_over_ids f_names @@
            match comps.Function_components.tau_opt with
            | Some tau when Splay.is_yes do_type_splay -> EGen tau
            | _ ->
              (* default behavior uses the actual function body *)
              build @@
              let ty =
                Binding.Ty.typed_of_tau_opt ~do_check:false comps.tau_opt
              in
              let* () = assign ~ty comps.func_id (
                  abstract_over_ids comps.params comps.defn
                )
              in
              return (EVar comps.func_id)
          ) func_comps
      in
      Expr.SUntyped { var = r ; defn = appl_list (Desugared_functions.y_n f_names) defns }
      :: List.map (fun comps ->
          (* do_check and do_wrap are unused arguments in this case because we don't provide the type *)
          make_stmt ~do_wrap:true ~do_check:true ~tau_opt:None comps.Function_components.func_id
          @@ proj (EVar r) (RecordLabel.RecordLabel comps.func_id)
        ) func_comps
      @ List.map (fun comps ->
          if Option.is_some comps.Function_components.tau_opt && Splay.is_yes do_type_splay then
            make_stmt ~do_wrap:true ~do_check:true ~tau_opt:comps.tau_opt comps.func_id (
              abstract_over_ids comps.params comps.defn (* actual definition of function *)
            )
          else
            (* no type or not splaying, so the actual definition was used above, so just project out from the record *)
            make_stmt ~do_wrap:false ~do_check:true ~tau_opt:comps.tau_opt comps.func_id (EVar comps.func_id)
        ) func_comps

  and desugar_statement (stmt : Bluejay.statement) : Desugared.statement list =
    match stmt with
    | SUntyped { var ; defn } ->
      [ Expr.SUntyped { var ; defn = desugar defn } ]
    | STyped { typed_var = { var ; tau } ; defn ; typed_binding_opts = TBBluejay } ->
      [ Expr.STyped {
            typed_var = { var ; tau = desugar tau };
            defn = desugar defn;
            typed_binding_opts = TBDesugared { do_wrap = true; do_check = true }
          }
      ]
    | SFun fsig ->
      let { Function_components.func_id ; defn ; params ; tau_opt } =
        funsig_to_components fsig
      in
      let defn = abstract_over_ids params defn in
      [ make_stmt ~do_wrap:true ~do_check:true ~tau_opt func_id defn ]
    | SFunRec fsigs ->
      desugar_rec_funs_to_stmt_list fsigs

  and make_stmt ~(do_wrap : bool) ~(do_check : bool) ~(tau_opt : Desugared.t option)
      (var : Ident.t) (defn : Desugared.t) : Desugared.statement =
    match tau_opt with
    | None ->
      SUntyped { var ; defn }
    | Some tau ->
      STyped {
        typed_var = { var ; tau };
        defn;
        typed_binding_opts = TBDesugared { do_wrap; do_check };
      }
  in

  List.concat_map desugar_statement pgm